...
def create_app(app_name=PKG_NAME, **kwargs):
    from classtype_backend.namespaces.api import api as apiNamespace

    application = Flask(app_name)
    CORS(application)
    api = Api(application,
              version='0.1',
              title='Classtype Backend API',
              description='Taxi Classtype Users Backend API')

    from classtype_backend.db import db, db_config
